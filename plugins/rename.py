...
async def rename_start(client, message):
    ...
    if message.from_user.id not in ADMINS:
        await message.reply_text(text="Only Admins can use me",chat_id=message.from_user.id)
...
async def refunc(client, message):
    ...
    elif (reply_message.reply_markup) and isinstance(reply_message.reply_markup, ForceReply):
...
async def doc(bot, update):
    ...
    if user_metadata_enabled == "On":
        ...
        metadata_command = [
            ...
            f"-vf drawtext=fontfile='font.ttf':fontsize=30:fontcolor=white:bordercolor=black@0.50:x=10:y=10:text='Hentai Arsenal'",
            '-map', '0',
            '-c', 'copy',
            '-loglevel', 'error',
            temp_output_file
    ...
    if c_caption:
        try:
            caption = c_caption.format(filename=new_filename_, filesize=humanbytes(media.file_size),
    ...
    try:
        if type == "document":
            await bot.send_document(
                ...
                document=file_path,
                file_name=file_nameu,
                thumb=ph_path,
                caption=f"<blockquote><b>{new_filename_}</b></blockquote>",
